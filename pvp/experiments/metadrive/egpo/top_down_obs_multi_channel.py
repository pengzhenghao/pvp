...
class TopDownMultiChannel(TopDownObservation):
    ...
    def observe(self, vehicle: BaseVehicle, render=True):
        if render: #and self._should_draw_map:
            self.render()
        ...
        return {self.IMAGE: np.transpose(img, (1, 0, 2)), self.STATE: self.state_obs.observe(vehicle)}
    ...
    def observation_space(self):
        ...
        if self.norm_pixel:
            imgspace = gym.spaces.Box(-0.0, 1.0, shape=shape, dtype=np.float32)
        else:
            imgspace = gym.spaces.Box(0, 255, shape=shape, dtype=np.uint8)
