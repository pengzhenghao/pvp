...
class HumanInTheLoopEnv(SafeMetaDriveEnv):
    ...
    def step(self, actions):
        ...
        if hasattr(self, "model") and hasattr(self.model, "trained"):
            ...
            self.ours = self.compute_uncertainty(actions)
            self.ours2 = var_actions - self.model.switch2human_thresh
            
            #self.compute_uncertainty(actions)
            ...
            self.ours3 = decision #max(act_diff - self.config['thr_actdiff'], var_actions - self.model.switch2human_thresh)
            self.ours4 = max(0, decision) #max(act_diff - self.config['thr_actdiff'], var_actions - self.model.switch2human_thresh))
        else:
            act_diff = 0
            var_actions = 0
            decision = 1
            self.ours, self.ours2,self.ours3,self.ours4 = 0,0,0,0
        ...
        if self.config["use_render"]:  # and self.config["main_exp"]: #and not self.config["in_replay"]:
             super(HumanInTheLoopEnv, self).render(
                 text={
                     "Takeover": "YES" if self.takeover else "NO",
        ...
        ret[-1]["ours"] = self.ours 
        ret[-1]["ours2"] = self.ours2
        ret[-1]["ours3"] = self.ours3
        ret[-1]["ours4"] = self.ours4
        return ret
