...
class PVPTD3_PREF(TD3):
    def __init__(self, use_balance_sample=True, q_value_bound=1.,
        ...
        *args, **kwargs):
        ...
        super(PVPTD3_PREF, self).__init__(*args, **kwargs)
    ...
    def _setup_model(self) -> None:
        super(PVPTD3_PREF, self)._setup_model()
        if self.use_balance_sample:
            self.human_data_buffer = HACOReplayBuffer(
                self.buffer_size,
                self.observation_space,
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for step in range(gradient_steps):
            ...
            if self.prefreplay_buffer.pos > 0:
                replay_data_pref = self.prefreplay_buffer.sample(batch_size, env=self._vec_normalize_env)
            else:
                loss = None
                break
    ...
    def _store_transition(
        ...
        if infos[0]["takeover"] or infos[0]["takeover_start"]:
            replay_buffer = self.human_data_buffer
        super(PVPTD3_PREF, self)._store_transition(replay_buffer, buffer_action, new_obs, reward, dones, infos)
    ...
    def save_replay_buffer(
        ...
        save_to_pkl(path_human, self.human_data_buffer, self.verbose)
        super(PVPTD3_PREF, self).save_replay_buffer(path_replay)
    ...
    def load_replay_buffer(
        ...
        if not hasattr(self.human_data_buffer, "handle_timeout_termination"):  # pragma: no cover
            self.human_data_buffer.handle_timeout_termination = False
            self.human_data_buffer.timeouts = np.zeros_like(self.replay_buffer.dones)
        super(PVPTD3_PREF, self).load_replay_buffer(path_replay, truncate_last_traj)
